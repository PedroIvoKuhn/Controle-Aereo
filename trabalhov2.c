...
#define SEGUNDOS_QUEDA_AVIAO 15  
#define MAX_VOOS 200
...
int solicitar_recurso(InfoVoo *voo, const char* nome_recurso, pthread_mutex_t *mutex, pthread_cond_t *cond_intl, pthread_cond_t *cond_dom, int *recurso_disponivel, int *esperando_intl);
void liberar_recurso(const char* nome_recurso, pthread_mutex_t *mutex, pthread_cond_t *cond_intl, pthread_cond_t *cond_dom, int *recurso_disponivel, int *esperando_intl);
...
int main() {
    ...
    printf("--- LOGICA DE PRIORIDADE ATIVA PARA PISTAS, PORTOES E TORRE ---\n");
    ...
    printf("Nenhum novo voo sera criado. Aguardando operaçoes em andamento...\n");
    simulacao_rodando = 0;
...
void* ciclo_vida_aviao(void* arg) {
    ...
    if (voo->tipo == INTERNACIONAL) {
        if (!solicitar_recurso(voo, "Pista", &aeroporto.mutex_pista, &aeroporto.cond_pista_intl, &aeroporto.cond_pista_dom, &aeroporto.pistas_disponiveis, &aeroporto.esperando_pista_intl)) return NULL;
        if (!solicitar_torre(voo)) { liberar_recurso("Pista", &aeroporto.mutex_pista, &aeroporto.cond_pista_intl, &aeroporto.cond_pista_dom, &aeroporto.pistas_disponiveis, &aeroporto.esperando_pista_intl); return NULL; }
    } else {
        if (!solicitar_torre(voo)) return NULL;
        if (!solicitar_recurso(voo, "Pista", &aeroporto.mutex_pista, &aeroporto.cond_pista_intl, &aeroporto.cond_pista_dom, &aeroporto.pistas_disponiveis, &aeroporto.esperando_pista_intl)) { liberar_torre(voo); return NULL; }
    }
    
    log_evento(voo->id, voo->tipo, "pista e torre alocadas. Pousando...");
    sleep(3);
    liberar_recurso("Pista", &aeroporto.mutex_pista, &aeroporto.cond_pista_intl, &aeroporto.cond_pista_dom, &aeroporto.pistas_disponiveis, &aeroporto.esperando_pista_intl);
    log_evento(voo->id, voo->tipo, "pouso concluído. Pista liberada.");
    liberar_torre(voo);

    // DESEMBARQUE
    voo->status = DESEMBARCANDO;
    if (!solicitar_recurso(voo, "Portao", &aeroporto.mutex_portao, &aeroporto.cond_portao_intl, &aeroporto.cond_portao_dom, &aeroporto.portoes_disponiveis, &aeroporto.esperando_portao_intl)) return NULL;
    if (!solicitar_torre(voo)) { liberar_recurso("Portao", &aeroporto.mutex_portao, &aeroporto.cond_portao_intl, &aeroporto.cond_portao_dom, &aeroporto.portoes_disponiveis, &aeroporto.esperando_portao_intl); return NULL; }

    log_evento(voo->id, voo->tipo, "portao e torre alocados. Desembarcando...");
    liberar_torre(voo);
    sleep(5);
    log_evento(voo->id, voo->tipo, "desembarque concluído. Portao continua ocupado.");
    ...
    if (voo->tipo == INTERNACIONAL) {
        if (!solicitar_recurso(voo, "Pista", &aeroporto.mutex_pista, &aeroporto.cond_pista_intl, &aeroporto.cond_pista_dom, &aeroporto.pistas_disponiveis, &aeroporto.esperando_pista_intl)) { liberar_recurso("Portao", &aeroporto.mutex_portao, &aeroporto.cond_portao_intl, &aeroporto.cond_portao_dom, &aeroporto.portoes_disponiveis, &aeroporto.esperando_portao_intl); return NULL; }
        if (!solicitar_torre(voo)) { 
            liberar_recurso("Portao", &aeroporto.mutex_portao, &aeroporto.cond_portao_intl, &aeroporto.cond_portao_dom, &aeroporto.portoes_disponiveis, &aeroporto.esperando_portao_intl);
            liberar_recurso("Pista", &aeroporto.mutex_pista, &aeroporto.cond_pista_intl, &aeroporto.cond_pista_dom, &aeroporto.pistas_disponiveis, &aeroporto.esperando_pista_intl);
            return NULL; 
        }
    } else {
    ...
    log_evento(voo->id, voo->tipo, "ciclo de operaçoes finalizado com sucesso.");
...
int solicitar_recurso(InfoVoo *voo, const char* nome_recurso, pthread_mutex_t *mutex, pthread_cond_t *cond_intl, pthread_cond_t *cond_dom, int *recurso_disponivel, int *esperando_intl) {
    char msg[100];
    sprintf(msg, "solicitando %s.", nome_recurso);
    log_evento(voo->id, voo->tipo, msg);
    ...
    if (voo->tipo == INTERNACIONAL) {
        (*esperando_intl)++;
        while (*recurso_disponivel == 0) { // Espera recursos
            pthread_cond_wait(cond_intl, mutex);
        }
        (*esperando_intl)--;
    } else { // DOMESTICO
        voo->tempo_espera_inicio = time(NULL);
        
        while (*recurso_disponivel == 0 || *esperando_intl > 0) { // Espera se não houver recurso OU se um internacional estiver esperando.
            struct timespec timeout;
            clock_gettime(CLOCK_REALTIME, &timeout);
            timeout.tv_sec += 1;

            int rc = pthread_cond_timedwait(cond_dom, mutex, &timeout);
            if (rc == ETIMEDOUT) {
                time_t tempo_espera = time(NULL) - voo->tempo_espera_inicio;
                if (tempo_espera >= SEGUNDOS_QUEDA_AVIAO) {
                    sprintf(msg, "CAIU! Tempo de espera por %s excedeu o limite. Falha operacional.", nome_recurso);
                    log_evento(voo->id, voo->tipo, msg);
                    voo->status = ACIDENTE;
                    pthread_mutex_lock(&stats.mutex_stats);
                    ...
                    sprintf(msg, "MAYDAY! MAYDAY! Preciso de %s", nome_recurso);
                    log_evento(voo->id, voo->tipo, msg);
                    pthread_mutex_lock(&stats.mutex_stats);
...
void liberar_recurso(const char* nome_recurso, pthread_mutex_t *mutex, pthread_cond_t *cond_intl, pthread_cond_t *cond_dom, int *recurso_disponivel, int *esperando_intl) {
    ...
    if (*esperando_intl > 0) { // Acorda voos internacionais primeiro
    ...
    if (voo->tipo == INTERNACIONAL) {
        ...
        while (aeroporto.usuarios_torre >= TORRE_CAPACIDADE || aeroporto.esperando_torre_intl > 0) { // Espera se a torre está cheia OU outro avião internacional na fila
            ...
            if (rc == ETIMEDOUT) {
                ...
                if (tempo_espera >= SEGUNDOS_QUEDA_AVIAO) {
                    log_evento(voo->id, voo->tipo, "CAIU!");
                    voo->status = ACIDENTE;
                    pthread_mutex_lock(&stats.mutex_stats);
                    stats.voos_acidentados++;
                    pthread_mutex_unlock(&stats.mutex_stats);
                    ...
                    log_evento(voo->id, voo->tipo, "MAYDAY! MAYDAY!");
                    pthread_mutex_lock(&stats.mutex_stats);
                    stats.alertas_starvation++;
                    pthread_mutex_unlock(&stats.mutex_stats);
                }
